...
- Integration setup (use the `## Integrations` directory — or `searchIntegrations` when that list is absent — first, then docs for additional context)
...
- For integration setup, prefer the `## Integrations` directory (or `searchIntegrations` when that list is absent from your instructions)
