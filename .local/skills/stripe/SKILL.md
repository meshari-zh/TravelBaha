...
2. The Stripe integration must be connected to the repl. Propose it when the `## Integrations` directory does not already list it as `added`; when that list is absent from your instructions, check with `searchIntegrations` first.
