...
2. The RevenueCat integration must be connected to the repl. This is necessary as the integration creates an authenticated client for RevenueCat accessible via a function `getUncachableRevenueCatClient`. Propose it when the `## Integrations` directory does not already list it as `added`; when that list is absent from your instructions, check with `searchIntegrations` first.
