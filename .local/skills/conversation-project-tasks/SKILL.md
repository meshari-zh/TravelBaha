...
`listProjects` (see the `user-projects` skill) finds the target project; use `` `${ownerSlug}/${slug}` `` from its results as the `projectSlug` for every call below.
...
`{ task }` -- with `taskRef`, `title`, `description`, `state`, `displayState`, `dependsOn`, `artifactKinds`, `recommendedModelProfile`, `createdAt`, `updatedAt`
...
| `tasks` | list | Yes | 1-100 items, each `{ title, description, alias?, dependsOn?, artifactKinds?, recommendedModelProfile? }` |
| `offerToStart` | bool | No | Defaults to true. Pass false only when the user explicitly wants drafts without starting them. |

Each task's `description` is the inline markdown plan (what & why, what done looks like, steps). `dependsOn` entries may be existing task refs or the `alias` of another task in the same batch. `recommendedModelProfile` is the model profile you recommend for running the task: `economy` for routine, well-scoped work; `power` for complex or ambiguous work (defaults to `economy`).
...
### updateProjectTask({ projectSlug, taskRef, title?, description?, dependsOn?, artifactKinds?, recommendedModelProfile? })

Edit an existing task. `description` replaces the whole plan; `dependsOn` is a full replacement list (`[]` clears all dependencies). `recommendedModelProfile` (`economy` or `power`) updates the run-tier recommendation; omit to leave it unchanged. Pass at least one field.
