...
Each search is one webSearch({ query: "<one full question>" }) call -- query is a REQUIRED string (there is no queries parameter) and results come back in .resultPages. webSearch and webFetch are pre-registered callbacks in your CodeExecution environment; call them directly, nothing to import.
