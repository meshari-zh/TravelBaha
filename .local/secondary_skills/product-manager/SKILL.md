...
- **Connected tools**: Check if the workspace has project management integrations (Linear, Jira, etc.) via the `## Integrations` directory (or `searchIntegrations` when that list is absent from your instructions). If connected, format output to match the tool's conventions (e.g., Linear project spec format for Linear users) and mention that items can be copied into their tool
